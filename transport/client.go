...
func NewClient(baseURL, token string) (*Client, error) {
	...
	return &Client{BaseURL: normalizedURL, Token: token, HTTPClient: http.DefaultClient}, nil
}

// NewClientFromEnv reads CHESS_NETWORK_URL and CHESS_NETWORK_TOKEN.
func NewClientFromEnv() (*Client, error) {
	return NewClient(os.Getenv("CHESS_NETWORK_URL"), os.Getenv("CHESS_NETWORK_TOKEN"))
}
...
func (c *Client) Do(ctx context.Context, messageType protocol.MessageType, requestID string, payload any) (protocol.Envelope, error) {
	...
	request, err := http.NewRequestWithContext(ctx, http.MethodPost, c.BaseURL+"/v1/messages", strings.NewReader(string(body)))
	if err != nil {
		return protocol.Envelope{}, err
	}
